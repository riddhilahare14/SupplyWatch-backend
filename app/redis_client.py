...
async def set_shipment_location(shipment_id: str, lat: float, lng: float,
    ...
    await r.set(f"shipment:{shipment_id}:location", data, ex=settings.CACHE_TTL)
...
async def set_shipment_state(shipment_id: str, state: Dict[str, Any]) -> None:
    r = await get_redis()
    await r.set(f"shipment:{shipment_id}:state", json.dumps(state), ex=settings.CACHE_TTL)
...
async def set_shipment_features(shipment_id: str, features: Dict[str, Any]) -> None:
    r = await get_redis()
    await r.set(f"shipment:{shipment_id}:features", json.dumps(features), ex=settings.CACHE_TTL)
...
async def push_gps_ping(shipment_id: str, ping_data: Dict) -> None:
    """Push a GPS ping to the rolling list.
    Keeps the last GPS_PING_BUFFER_SIZE pings (default 60 ≈ 10 min at 10-s intervals).
    """
    r = await get_redis()
    key = f"shipment:{shipment_id}:pings"
    await r.lpush(key, json.dumps(ping_data))
    await r.ltrim(key, 0, settings.GPS_PING_BUFFER_SIZE - 1)


async def get_recent_pings(shipment_id: str, count: int = None) -> list:
    """Get the last N GPS pings for a shipment.
    Defaults to GPS_PING_BUFFER_SIZE if count is not specified.
    """
    if count is None:
        count = settings.GPS_PING_BUFFER_SIZE
    r = await get_redis()
    raw_list = await r.lrange(f"shipment:{shipment_id}:pings", 0, count - 1)
    return [json.loads(p) for p in raw_list]