...
# ─────────────────────────
# Pings the Render deployment's health endpoint every 30 s so the
# free-tier dyno never idles past the 50-second inactivity window.
...
# # Override via env var SELF_URL if you rename the service.
# _DEFAULT_URL = "https://supplywatch-backend.onrender.com/"
# SELF_URL: str = os.getenv("SELF_URL", _DEFAULT_URL)
# PING_INTERVAL: int = int(os.getenv("KEEPALIVE_INTERVAL", "30"))   # seconds
...
#     """Infinite loop: GET SELF_URL every PING_INTERVAL seconds."""
#     logger.info(
#         "Keep-alive started → pinging %s every %ds", SELF_URL, PING_INTERVAL
#     )
...
#                 logger.debug("Keep-alive ping → %s %s", resp.status_code, SELF_URL)
...
#     """Schedule the ping loop and return the Task (so lifespan can cancel it)."""
